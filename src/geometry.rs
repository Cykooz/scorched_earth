...
impl Circle {
    ...
    pub fn line_intersection<P>(&self, point1: P, point2: P) -> Vec<Point2>
    where
        P: Into<Point2>,
    {
        // Translate the line into the coordinate system relative to the center of the circle.
        let point1: Vector2 = point1.into() - self.center;
        let point2: Vector2 = point2.into() - self.center;

        let line_vector: Vector2 = point2 - point1;
        let dr2 = line_vector.magnitude2();
    ...
    pub fn segment_intersection<P>(&self, point1: P, point2: P) -> Vec<Point2>
    where
        P: Into<Point2>,
    {
        ...
        let segment_vector: Vector2 = point2 - point1;
        let segment_len2 = segment_vector.magnitude2();
        result
            .into_iter()
            .filter(|res_point| {
                let res_vector: Vector2 = res_point - point1;
    ...
    pub fn area_of_rect_intersection(&self, mut rect: Rect) -> f32 {
        rect.x -= self.center.x;
        rect.y -= self.center.y;

        self.area_of_normalized_rect_intersection(rect)
    }

    fn area_of_normalized_rect_intersection(&self, mut rect: Rect) -> f32 {
        if rect.top() < 0.0 {
            if rect.bottom() < 0.0 {
...
mod tests {
    ...
    fn test_circle_line_two_intersections() {
        ...
        assert_eq!(res.len(), 2);
        assert_eq!(res, vec![Point2::new(-2.0, 6.0), Point2::new(4.0, 6.0)]);

        // Line intersect lower half of circle
        let res = circle.line_intersection([0.0, -2.0], [1.0, -2.0]);
        assert_eq!(res.len(), 2);
        assert_eq!(res, vec![Point2::new(-2.0, -2.0), Point2::new(4.0, -2.0)]);

        // Line intersect left half of circle
        let res = circle.line_intersection([-3.0, 0.0], [-3.0, 1.0]);
        assert_eq!(res.len(), 2);
        assert_eq!(res, vec![Point2::new(-3.0, -1.0), Point2::new(-3.0, 5.0)]);

        // Line intersect right half of circle
        let res = circle.line_intersection([5.0, 0.0], [5.0, 1.0]);
        assert_eq!(res.len(), 2);
        assert_eq!(res, vec![Point2::new(5.0, -1.0), Point2::new(5.0, 5.0)]);

        // Line intersect center of circle
        let res = circle.line_intersection([0.0, 2.0], [1.0, 2.0]);
        assert_eq!(res.len(), 2);
        assert_eq!(res, vec![Point2::new(-4.0, 2.0), Point2::new(6.0, 2.0)]);
    }
    ...
    fn test_area_of_rect_intersection() {
        ...
        let area = circle.area_of_rect_intersection(Rect::new(-10.0, -10.0, 20.0, 20.0));
        assert_eq!(area, PI);

        // half of unit circle inside a large box, area of intersection is pi/2
        let area = circle.area_of_rect_intersection(Rect::new(-10.0, -10.0, 10.0, 20.0));
        assert_eq!(area, FRAC_PI_2);

        // half of unit circle inside a large box, area of intersection is pi/2
        let area = circle.area_of_rect_intersection(Rect::new(0.0, -10.0, 10.0, 20.0));
        assert_eq!(area, FRAC_PI_2);

        // half of unit circle inside a large box, area of intersection is pi/2
        let area = circle.area_of_rect_intersection(Rect::new(-10.0, -10.0, 20.0, 10.0));
        assert_eq!(area, FRAC_PI_2);

        // half of unit circle inside a large box, area of intersection is pi/2
        let area = circle.area_of_rect_intersection(Rect::new(-10.0, 0.0, 20.0, 10.0));
        assert_eq!(area, FRAC_PI_2);

        // unit box covering one quadrant of the circle, area of intersection is pi/4
        let area = circle.area_of_rect_intersection(Rect::new(0.0, 0.0, 1.0, 1.0));
        assert_eq!(area, FRAC_PI_4);

        // unit box covering one quadrant of the circle, area of intersection is pi/4
        let area = circle.area_of_rect_intersection(Rect::new(-1.0, 0.0, 1.0, 1.0));
        assert_eq!(area, FRAC_PI_4);

        // unit box covering one quadrant of the circle, area of intersection is pi/4
        let area = circle.area_of_rect_intersection(Rect::new(-1.0, -1.0, 1.0, 1.0));
        assert_eq!(area, FRAC_PI_4);

        // unit box covering one quadrant of the circle, area of intersection is pi/4
        let area = circle.area_of_rect_intersection(Rect::new(0.0, -1.0, 1.0, 1.0));
        assert_eq!(area, FRAC_PI_4);

        // huge box completely outside a circle (left), area of intersection is 0
        let area = circle.area_of_rect_intersection(Rect::new(-20.0, -10.0, 10.0, 20.0));
        assert_eq!(area, 0.0);
        ...
        let area = circle.area_of_rect_intersection(Rect::new(-10.0, -20.0, 20.0, 10.0));
        assert_eq!(area, 0.0);
        ...
        let area = circle.area_of_rect_intersection(Rect::new(-0.5, -0.5, 1.0, 1.0));
        assert_eq!(area, 1.0);

        let circle = Circle::new((87.0, 489.0), 50.0);
        let area = circle.area_of_rect_intersection(Rect::new(100.0, 466.0, 41.0, 41.0));
        assert!(area > 0.0);
    }
}
